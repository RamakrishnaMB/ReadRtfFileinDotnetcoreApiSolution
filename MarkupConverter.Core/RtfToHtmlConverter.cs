...
namespace MarkupConverter.Core
{
    public static class RtfToHtmlConverter
    {
        ...
        public static string ConvertRtfToHtmlMS(MemoryStream rtfMs)
        {
            string xamlText = $"<FlowDocument>{ConvertRtfToXamlMemoryStream(rtfMs)}</FlowDocument>";

            return HtmlFromXamlConverter.ConvertXamlToHtml(xamlText, asFullDocument: false);
        }

        private static string ConvertRtfToXamlMemoryStream(MemoryStream rtfMs)
        {
            var flowDocument = new FlowDocument();
            var textRange = new TextRange(flowDocument.ContentStart, flowDocument.ContentEnd);

            // Create a MemoryStream of the RTF content
            using (var rtfStreamReader = new StreamReader(rtfMs))
            {
                textRange = new TextRange(flowDocument.ContentStart, flowDocument.ContentEnd);
                string test = textRange.Text.ToString();
                textRange.Save(rtfMs, DataFormats.Xaml);
                rtfMs.Position = 0;
